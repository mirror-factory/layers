    ...
    <div className="space-y-8">
      ...
      <div className="rounded-lg border border-white/10 bg-white/[0.02] px-6 py-5">
        ...
        <div className="mt-4 flex flex-wrap gap-8 text-sm">
          {/* Run date */}
          <div>
            <p className="text-xs uppercase tracking-wider text-ink-200/40 mb-0.5">
              Run Date
            </p>
          ...
          <div>
            ...
            <p className="font-mono text-ink-200/80">{run.model}</p>
          </div>
          ...
          <div>
            ...
            <p className="font-mono">
              <span className="text-layers-mint">{passedCases} passed</span>
              {failedCases > 0 && (
                <>
                  {" / "}
                  <span className="text-signal-live">{failedCases} failed</span>
                </>
              )}
              {" / "}
              {totalCases} total
                ...
                <>
                  <tr
                    ...
                    className={`border-b cursor-pointer transition-colors ${
                      !c.passed
                        ? "border-signal-live/30 bg-signal-live/5 hover:bg-signal-live/10"
                  ...
                  >
                    <td className="px-4 py-3 text-ink-200/80 font-medium">
                      {c.name}
                    </td>
                    <td className="px-4 py-3 text-center">
                      {c.passed ? (
                        <span className="inline-block rounded px-2 py-0.5 text-xs font-medium bg-layers-mint/10 text-layers-mint border border-layers-mint/20">
                          PASS
                        </span>
                      ) : (
                        <span className="inline-block rounded px-2 py-0.5 text-xs font-medium bg-signal-live/10 text-signal-live border border-signal-live/20">
                          FAIL
                        </span>
                      )}
                    </td>
                    <td className="px-4 py-3 text-ink-200/60 text-xs max-w-[160px] truncate">
                      {truncate(c.input, 60)}
                    </td>
                    <td className="px-4 py-3 text-ink-200/60 text-xs max-w-[160px] truncate">
                    ...
                    <td className="px-4 py-3 text-center">
                      {c.traceId ? (
                        <Link
                          ...
                          className="text-layers-mint text-xs hover:underline"
                          onClick={(e) => e.stopPropagation()}
                        >
                          view
                        </Link>
                      ) : (
                        <span className="text-ink-200/20 text-xs">--</span>
                      )}
                    </td>
                  </tr>
                    ...
                    <tr key={`${c.id}-detail`}>
                      <td
                        ...
                        className={`bg-white/[0.02] ${
                          !c.passed
                            ? "border-l-2 border-l-signal-live"
                      ...
                      >
                        <div className="px-6 py-5 space-y-4">
                          ...
                          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                            <div>
                              ...
                              <pre
                                className={`text-xs rounded p-3 whitespace-pre-wrap max-h-48 overflow-y-auto ${
                                  !c.passed
                                    ? "bg-layers-mint/5 border border-layers-mint/20"
                            ...
                            <div>
                              ...
                              <pre
                                className={`text-xs rounded p-3 whitespace-pre-wrap max-h-48 overflow-y-auto ${
                                  !c.passed
                                    ? "bg-signal-live/5 border border-signal-live/20"
                                    : "bg-[#050505] border border-white/10"
                                }`}
                              >
                                {c.actual}
                          ...
                          <div className="flex flex-wrap gap-6 text-xs">
                            <div>
                              <span className="text-ink-200/50">Score: </span>
                              <span
                            ...
                            <div>
                              <span className="text-ink-200/50">Duration: </span>
                            ...
                            {c.traceId && (
                              <div>
                                <span className="text-ink-200/50">Trace: </span>
                                <Link
                                  href={`/dev-kit/sessions/${c.traceId}`}
                                  className="text-layers-mint font-mono hover:underline"
                                >
                                  {c.traceId}
                                </Link>
                              </div>
      ...
      {/* Summary footer */}
      <div className="flex items-center justify-between text-xs text-ink-200/40">
        <p>
