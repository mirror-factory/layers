...
export function localToolRegistry(cwd = process.cwd()) {
  ...
  for (const raw of readFileSync(file, "utf-8").split("\n")) {
    ...
    if (line.startsWith("has_test:"))
      current.testStatus =
        parseScalar(line.slice("has_test:".length)) === "true"
          ? "passing"
          : "untested";
...
function normalizeLocalTool(
  tool: Partial<ReturnType<typeof localToolRegistry>[number]>,
  index: number,
) {
  ...
  return {
    ...
    description:
      tool.description ||
      "Registered local AI tool from .ai-dev-kit/registries/tools.yaml.",
    category: path.includes("/mcp/") ? "mcp" : "ai-tool",
    permissionTier: path.includes("/mcp/")
      ? ("executor" as const)
      : ("explorer" as const),
    ...
    testFilePath:
      tool.testStatus === "passing" ? ".test-results/eval-results.json" : null,
    evalHistory: localEvalSuccess()
      ? [{ date: new Date().toISOString().slice(0, 10), score: 100 }]
      : [],
    path,
  };
}
...
export function localEvalSuccess(cwd = process.cwd()) {
  const evals = safeJson<{ success?: unknown }>(
...
export function localOverview(cwd = process.cwd()) {
  const plans = filesIn(join(cwd, ".ai-starter", "plans"), (name) =>
    name.endsWith(".json"),
  );
  const reports = filesIn(join(cwd, ".ai-starter", "reports"), (name) =>
    name.endsWith(".md"),
  );
  const scorecards = filesIn(
    join(cwd, ".ai-starter", "runs"),
    (name) => name.startsWith("scorecard-") && name.endsWith(".json"),
  );
    ...
    modules: [
      {
        name: "AI Starter Kit",
        status: "configured",
        description: ".ai-starter manifests are present",
      },
      {
        name: "AI DevKit",
        status: "configured",
        description: ".ai-dev-kit registries are present",
      },
      {
        name: "Local proof",
        status: localEvalSuccess(cwd) ? "passing" : "pending",
        description: ".test-results/eval-results.json",
      },
    ],
    source: "local-files",
  };
...
export function localSessions(cwd = process.cwd()) {
  return filesIn(
    join(cwd, ".ai-starter", "reports"),
    (name) => name.startsWith("report-") && name.endsWith(".md"),
  )
    .slice(0, 25)
    .map((file) => ({
    ...
    suites: [
      {
        ...
        cases: [
          {
            name: "Tool eval coverage",
            passed: evals?.success === true,
            score: evals?.success ? 1 : 0,
            durationMs: 0,
          },
        ],
      },
...
export function localDeployments(cwd = process.cwd()) {
  const latest = safeJson<{ score?: number; createdAt?: string }>(
    join(cwd, ".ai-starter", "runs", "latest-scorecard.json"),
  );
  return [
    {
...
export function localRegressions() {
  return [
    {
      id: "route-coverage",
...
export function localConnectors(cwd = process.cwd()) {
  ...
  return registries.map((name) => {
    ...
    return {
      ...
      lastSync: existsSync(file)
        ? statSync(file).mtime.toISOString()
        : new Date().toISOString(),
      errorCount: 0,
      description: existsSync(file)
        ? `Local registry ${name}.json is present.`
        : `Local registry ${name}.json is missing.`,
    };
  });
}
