...
import { hasSupabaseEnv, localOverview } from "../local-fallbacks";
...
export async function GET() {
  try {
    ...
    if (langfusePublicKey && langfuseSecretKey) {
      try {
        ...
        const tracesRes = await fetch(
          `${langfuseBaseUrl}/api/public/traces?limit=100`,
          { headers: { Authorization: `Basic ${auth}` } },
        );
        if (tracesRes.ok) {
          const traces = await tracesRes.json();
          const traceList = traces.data ?? [];
          const totalCost = traceList.reduce(
          ...
          const avgLatency =
            traceList.length > 0
              ? traceList.reduce(
                  (sum: number, t: any) => sum + (t.latency ?? 0),
                  0,
                ) / traceList.length
              : 0;
    ...
    if (!hasSupabaseEnv()) return NextResponse.json(localOverview());
    ...
    return NextResponse.json(localOverview());
  }
}
