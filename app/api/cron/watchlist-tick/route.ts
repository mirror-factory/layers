...
function checkCronAuth(req: Request): AuthResult {
  ...
  if (!cronSecret && !adminToken) {
    return isProd
      ? { ok: false, reason: "missing_token_in_prod" }
      : { ok: true };
  }
...
async function handle(req: Request): Promise<Response> {
  ...
  const dryRun =
    queryParse.data.dryRun === "1" || queryParse.data.dryRun === "true";
  ...
  if (dryRun) {
    ...
    return NextResponse.json(WatchlistTickResponseSchema.parse(payload), {
